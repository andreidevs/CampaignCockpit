...
| `SEED_BIG` | 0 | пилотов полного размера в каждую крупную ячейку до основной разведки; не прошёл 270 миров |
| `TAIL_K`, `TAIL_BIG_ONLY` | 1.0, `True` | множитель на sd сдвига от пилота в KG (тяжёлый хвост эффектов); не прошёл 270 миров |
| `TAIL_UNRELIABLE` | 1.0 | TAIL_K во всех ячейках, если после `ADAPT_AFTER` пилотов история признана ненадёжной; не прошёл быстрый стенд |
| `TARGET_SD`, `SEG_SD` | 0.0, 0.0 | общие эффекты целевого тарифа / ARPU-сегмента в совместной модели; не прошли 270 миров |
| `CORR_KG` | `False` | KG с корреляциями между рукавами; хуже на быстром стенде |
| `LCB_K` | 0.5 | в план идёт `μ − k·σ > 0` |
| `LCB_K_PILOT` | 0.5 | k для рукавов после пилота |
| `LCB_K_HIGH` | 0.5 | k для HIGH-ячеек в план (0.5 = как у остальных; 1.0 не прошёл gate) |
| `PILOT_POS_ONLY` | `False` | пилоты только на рукава с μ > 0: risk на моке 0%, не прошёл gate |
...
| `B2` | 0.0 | sd общего сдвига Δ% ARPU (эффект ∝ доле перехода); 0 — выключено, не прошёл 270 миров |
...
| `FLAT_PRIOR` | 0.0 | bma: доля априорного веса на «плоских» точках сетки (эффект не от доли); 0 — равный вес |
| `SCALE_EXPLORE` | `"none"` | `"widen"` (при `"ml"`): по ходу разведки масштаб только ≥ 1 |
...
| `PUSH_K` | 0.0 | push-рукав при μ − k·σ > 0; 0 — по ожиданию |
